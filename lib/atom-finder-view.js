...
export default class AtomFinderView {
  ...
  showPrompt( text, callback = null ) {
    ...
    var prompt_submit = function(event) {
      event.preventDefault();
      var input = $prompt.find('#'+handle ).val();
      callback.apply( self.model, [ input ]);
      $prompt.off('submit', prompt_submit );
      $prompt.remove();
      self.$element.toggleClass('waiting', false );
      $prompt = null;
    }

    $prompt.on('submit', prompt_submit );

    this.$element.toggleClass('waiting', true );
    this.$element.append( $prompt );
  }
